...
import { filterUserTasks } from "../../redux/features/tasks/tasksSlice";

const MyTasks = () => {
  // get all tasks and filter by user
  const { tasks, userTasks: userTaskArray } = useSelector((state) => state.taskSlice);
  const { name: userName } = useSelector((state) => state.userSlice);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(filterUserTasks(userName));
  }, [dispatch, userName]);
